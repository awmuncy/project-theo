...
/**
 * This is forked from https://github.com/alexreardon/tiny-invariant/blob/master/src/tiny-invariant.ts
 * We are doing this so we could control if the error message gets returned or not based on
 * INVARIANT_HIDE_MESSAGE instead of NODE_ENV
 */

export type Message = string | (() => string);
const hideMessage: boolean = process.env.INVARIANT_HIDE_MESSAGE === "true";
...
export default function invariant(
    ...
    // Don't include the message if INVARIANT_HIDE_MESSAGE is set
    if (hideMessage) {
